...
const NODES: NavNode[] = [
  { id: 'docs',         glyph: '▤', label: '/docs/',         title: 'The full handbook',    desc: 'Hardware · Net · Services', proto: 'HTTPS · 2ms', href: '/docs/infrastructure/hardware', isDoc: true, cx:  82, cy:  74 },
  { id: 'services',     glyph: '◈', label: '/services',      title: "What's running",       desc: 'Containers · VMs · Status', proto: 'WSS · live',  href: '/services',                                  cx: 478, cy:  74 },
  { id: 'architecture', glyph: '⬡', label: '/architecture/', title: 'How it fits together', desc: 'Topology · VMID · Subnets', proto: 'ICMP · 1ms',  href: '/architecture',                              cx:  82, cy: 306 },
  { id: 'changelog',    glyph: '◷', label: '/changelog',     title: 'What changed, when',   desc: 'Timeline · Decisions',      proto: 'GIT · 5ms',   href: '/docs/changelog',                            cx: 478, cy: 306 },
];
...
// midpoint of a gateway→node line
const mid = (n: NavNode) => ({ x: (GX + n.cx) / 2, y: (GY + n.cy) / 2 });

// ── gateway (router with antennas, ports, throughput) ──
function Gateway() {
  const portW = 15, portH = 10, gap = 4, nPorts = 6;
  const span  = nPorts * portW + (nPorts - 1) * gap;       // 110
  const portX = GX - span / 2;                             // 225
  const portY = GPY + GH - portH - 8;                      // 205

  return (
    <g>
      {/* antennas (behind body) */}
      {[-44, 44].map((dx, i) => {
        const bx = GX + dx, by = GPY + 6;
        const tx = GX + dx + (dx < 0 ? -8 : 8), ty = GPY - 18;
        return (
          <g key={i}>
            <line x1={bx} y1={by} x2={tx} y2={ty} stroke="rgba(255,255,255,0.22)" strokeWidth={2} strokeLinecap="round" />
      ...
      {/* status LED */}
      <circle cx={GPX + 13} cy={GPY + 15} r={3.5} fill="#22c55e" filter="url(#nn-glow)" className="nn-pulse" />

      {/* label */}
      <text x={GPX + 24} y={GPY + 19} fontSize={10} fontFamily="ui-monospace, monospace" fontWeight={700} fill="rgba(255,255,255,0.65)">
        JAYSYNC-GW
      </text>

      {/* throughput mini-graph (top-right) */}
      {[0, 1, 2, 3].map((i) => (
        <rect key={i} className="nn-bar" x={330 + i * 7} y={162} width={4} height={15} rx={1}
          fill="#22c55e" opacity={0.8} style={{ animationDelay: `${i * 0.18}s` }} />
      ))}

      {/* separator */}
      <line x1={GPX + 10} y1={GPY + 28} x2={GPX + GW - 10} y2={GPY + 28} stroke="rgba(255,255,255,0.07)" strokeWidth={0.5} />
      ...
      {/* IP caption */}
      <text x={GX} y={GPY + GH + 16} textAnchor="middle" fontSize={8.5} fontFamily="ui-monospace, monospace"
        fill="rgba(255,255,255,0.22)"
        style={{ paintOrder: 'stroke', stroke: '#080808', strokeWidth: 3, strokeLinejoin: 'round' } as React.CSSProperties}>
  ...
  return (
    ...
    >
      ...
      <div className="relative p-2 sm:p-3">
        ...
        <svg viewBox={`0 0 ${VW} ${VH}`} className="pointer-events-none w-full h-full" aria-hidden>
          <defs>
            ...
            <radialGradient id="nn-core" cx="50%" cy="50%" r="50%">
              ...
              <stop offset="55%"  stopColor="rgba(96,165,250,0.03)" />
              <stop offset="100%" stopColor="transparent" />
            </radialGradient>
            <pattern id="nn-grid" width={26} height={26} patternUnits="userSpaceOnUse">
              <path d="M26 0H0V26" fill="none" stroke="rgba(255,255,255,0.035)" strokeWidth={0.5} />
            </pattern>
          </defs>
          ...
          <style>{`
            ...
            .nn-bar   { transform-box: fill-box; transform-origin: bottom;
                        animation: nn-bar 1.1s ease-in-out infinite alternate }
          `}</style>

          {/* blueprint grid + center glow */}
          <rect x={0} y={0} width={VW} height={VH} fill="url(#nn-grid)" />
          <circle cx={GX} cy={GY} r={150} fill="url(#nn-core)" />

          {/* connection lines + traveling packets */}
          {NODES.map((n, i) => {
            const isHot = hot === n.id, col = ACCENT[n.id];
            return (
              <g key={n.id}>
                <line x1={GX} y1={GY} x2={n.cx} y2={n.cy}
                  className="nn-flow"
                ...
                <circle r={isHot ? 3 : 2.2} fill={col} opacity={isHot ? 1 : 0.5}
                  filter={isHot ? 'url(#nn-glow)' : undefined}>
                  <animateMotion dur="2.4s" begin={`${i * 0.5}s`} repeatCount="indefinite"
                    path={`M${GX},${GY} L${n.cx},${n.cy}`} />
                </circle>
              </g>
            );
          })}

          {/* per-route telemetry (hidden on mobile) */}
          <g className="hidden sm:block">
            {NODES.map((n) => {
              const m = mid(n), isHot = hot === n.id;
              return (
                <text key={n.id} x={m.x} y={m.y - 6} textAnchor="middle"
                  fontSize={8.5} fontFamily="ui-monospace, monospace"
                  fill={isHot ? ACCENT[n.id] : 'rgba(255,255,255,0.2)'}
                  style={{
                    transition: 'fill .18s',
                    paintOrder: 'stroke', stroke: '#080808', strokeWidth: 3, strokeLinejoin: 'round',
          ...
          {NODES.map((n) => {
            const isHot = hot === n.id, col = ACCENT[n.id];
            return (
              <rect key={n.id} x={n.cx - NW / 2} y={n.cy - NH / 2} width={NW} height={NH} rx={8}
        ...
        {NODES.map((n) => {
          ...
          return (
            <div key={n.id} className="absolute"
              style={{ left: px(n.cx - NW / 2, VW), top: px(n.cy - NH / 2, VH), width: px(NW, VW), height: px(NH, VH) }}
